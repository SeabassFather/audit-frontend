...
export default function TraceabilityTracker({ sampleType = 'water', onLocationSubmit }) {
  ...
  return (
    <div style={{background: '#1e293b', padding: '30px', borderRadius: '16px', border: '2px solid #06b6d4'}}>
      <h2 style={{color: '#06b6d4', marginBottom: '20px'}}>🌍 Location Tracking</h2>
      <button onClick={getGPS} style={{padding: '10px 20px', background: '#06b6d4', border: 'none', borderRadius: '8px', color: 'white', fontWeight: '700', cursor: 'pointer', marginBottom: '20px'}}>📍 Auto-Detect GPS</button>
      ...
      <button onClick={() => {
        if (!locationData.latitude || !locationData.longitude || !locationData.farmName || !locationData.fieldNumber) {
          alert('⚠️ GPS coordinates, farm name, and field number required!');
          return;
        }
      ...
      }} style={{width: '100%', marginTop: '20px', padding: '16px', background: 'linear-gradient(135deg, #06b6d4, #0891b2)', border: 'none', borderRadius: '12px', color: 'white', fontSize: '1.2rem', fontWeight: '700', cursor: 'pointer'}}>✅ Confirm Location</button>
    </div>
  );
}