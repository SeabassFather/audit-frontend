...
export default function FactoringPO() {
  ...
  const go = async (e) => {
    ...
    setOut(null);
    try {
      setOut(await apiPost("/api/search/trade-finance", p));
    } catch (ex) {
      setErr(String(ex));
    }
  };
    ...
    <section>
      ...
      <form onSubmit={go} className="grid grid-2">
        ...
        <input
          ...
          value={p.buyer}
          onChange={(e) => setP({ ...p, buyer: e.target.value })}
        />
        <input
          className="input"
          placeholder="Invoice / PO #"
          value={p.invoice}
          onChange={(e) => setP({ ...p, invoice: e.target.value })}
        />
        <input
          className="input"
          placeholder="Amount (USD)"
          value={p.amount}
          onChange={(e) => setP({ ...p, amount: Number(e.target.value) || 0 })}
        />
        <input
          className="input"
          placeholder="Goods / Description"
          value={p.goods}
          onChange={(e) => setP({ ...p, goods: e.target.value })}
        />
