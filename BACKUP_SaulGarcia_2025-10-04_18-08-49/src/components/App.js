...
function ServicesTab() {
  ...
  const categories = [
    ...
    {
      name: 'Insurance & Risk', icon: '🛡️', color: '#14b8a6',
    ...
    {
      name: 'Global Compliance', icon: '⚖️', color: '#8b5cf6',
      ...
      {categories.map((cat, idx) => (
        <div key={idx} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', marginBottom: '12px', overflow: 'hidden' }}>
          <button onClick={() => setExpandedCategory(expandedCategory === idx ? null : idx)}
            ...
            <div style={{ fontSize: '20px', color: '#64748b', transform: expandedCategory === idx ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.3s' }}>▼</div>
          </button>
...
function SearchEngines() {
  ...
  const engines = [
    ...
    { key: 'mexico', label: 'Mexico RE', labelEs: 'Bienes Raíces', icon: '🇲🇽' },
    { key: 'compliance', label: 'Compliance', labelEs: 'Cumplimiento', icon: '⚖️' },
