    ...
    <div style={{maxWidth: 900, margin: "24px auto", padding: "0 12px"}}>
      ...
      <div style={{marginTop:16}}>
        {filtered.map((sec, idx) => (
          <details key={idx} open style={{background:"#121212", border:"1px solid #2a2a2a", borderRadius:10, marginBottom:10}}>
            <summary style={{cursor:"pointer", padding:"10px 14px", fontWeight:600}}>{sec.title}</summary>
            <ul style={{margin:"6px 0 14px 24px"}}>
              {sec.items.map((it, i)=> <li key={i} style={{margin:"6px 0"}}>{it}</li>)}
            </ul>
          </details>
        ))}
      </div>
...
function Card({title, children}) {
  return (
    <div style={{background:"#121212", border:"1px solid #2a2a2a", borderRadius:12}}>
      <div style={{padding:"10px 14px", borderBottom:"1px solid #222", fontWeight:700}}>{title}</div>
...
function Row({name, price, avgRecovery, time}) {
  return (
    <div style={{display:"grid", gridTemplateColumns:"1.2fr .6fr .8fr .6fr", gap:8, padding:"8px 6px", borderBottom:"1px dashed #222"}}>
      <div>{name}</div>
