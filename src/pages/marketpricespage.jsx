...
const ALL_COMMODITIES = [
  ...
  { name: "Oranges (Navel)", nass: { commodity_desc: "ORANGES", statisticcat_desc: "PRICE RECEIVED", unit_desc: "DOLLARS / LB" } },
  // Add more as needed!
];

export default function MarketPricesPage() {
  const [query, setQuery] = useState("");
  const [selectedCommodity, setSelectedCommodity] = useState(ALL_COMMODITIES[0]);
    ...
    <div style={{ minHeight: "100vh", background: "#f8f9fc" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <span style={{ fontWeight: 700, fontSize: "1.22rem", color: "#cb356b", marginLeft: 24 }}>
            <span role="img" aria-label="logo" style={{ fontSize: "1.45rem", marginRight: 7 }}>🍉</span>
            AuditDNA
          </span>
          <span style={{
            color: "#888", fontSize: "1.02rem", fontWeight: 450, marginLeft: 20
          }}>
            Ticker: {selectedCommodity.name} W1W26 avg (real USDA API)
          </span>
        </div>
        <div style={{ marginRight: 30 }}>
          <a href="/services" style={headerLink}>Services</a>
          <a href="/cases" style={headerLink}>Cases</a>
          <a href="/market/prices" style={headerLink}>USDA Prices</a>
          <a href="/marketplace" style={headerLink}>Ag Marketplace</a>
          <a href="/admin" style={headerLink}>Admin</a>
          <span style={{
            marginLeft: 30,
            background: "#222",
            color: "#fff",
            ...
            <div style={{ display: "flex", gap: 11, flexWrap: "wrap" }}>
              {filteredCommodities.map(c => (
                <button
                  ...
                  style={{
                    background: selectedCommodity.name === c.name ? "#cb356b" : "#f3f3f6",
                    ...
                    fontSize: ".98rem",
                    padding: "8px 22px",
                    boxShadow: selectedCommodity.name === c.name ? "0 2px 10px rgba(203,53,107,0.09)" : "none",
                    cursor: "pointer",
                    transition: "all .13s"
                  }}
                >
