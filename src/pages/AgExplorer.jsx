...
const COMMODITY_HINTS = ["Papaya","Orange","Apple","Grape","Tomato","Avocado","Lemon","Lime","Mango","Banana"];
    ...
    <div className="p-4">
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
        {["produce","water","factoring","tickers"].map(t=>(
          <button key={t} className="btn" onClick={()=>setTab(t)}>{t.toUpperCase()}</button>
        ))}
      </div>
      ...
      {tab==="produce" && (
        <div className="card">
          ...
          <input className="filter" placeholder="Search commodity name" value={q} onChange={e=>setQ(e.target.value)} />
          {hints.length===0 ? <div className="empty">type to filter</div> : (
            <div className="card-grid" style={{marginTop:12}}>
              {hints.map(h=>(<div className="card" key={h}><div className="card-title">{h}</div><div className="subtext">connect USDA endpoint to show live data</div></div>))}
            </div>
          )}
        </div>
      )}
      ...
      {tab==="water" && (
        <div className="grid">
          <div className="card">
            ...
            <div className="kv"><span>Acres</span><input className="filter" type="number" value={acre} onChange={e=>setAcre(+e.target.value||0)} /></div>
            <div className="kv"><span>Flow (GPM)</span><input className="filter" type="number" value={gpm} onChange={e=>setGpm(+e.target.value||0)} /></div>
            <div className="kv"><span>Hours/day</span><input className="filter" type="number" value={hours} onChange={e=>setHours(+e.target.value||0)} /></div>
      ...
      {tab==="factoring" && (
        <div className="grid">
          <div className="card">
            ...
            <div className="kv"><span>Invoice $</span><input className="filter" type="number" value={invoice} onChange={e=>setInvoice(+e.target.value||0)} /></div>
            <div className="kv"><span>Rate % / 30d</span><input className="filter" type="number" value={rate} onChange={e=>setRate(+e.target.value||0)} /></div>
            <div className="kv"><span>Days</span><input className="filter" type="number" value={days} onChange={e=>setDays(+e.target.value||0)} /></div>
