...
const COMMODITIES = ["Tomatoes","Avocados","Limes","Bell Peppers","Cucumbers"];
...
function useAsync(fn, deps) {
  ...
  useEffect(()=>{ let alive=true; (async()=>{
      ...
      try { const data=await fn(); if(alive) set({loading:false,data,error:null}); }
      catch(e){ if(alive) set({loading:false,data:null,error:e}); }
  })(); return ()=>{ alive=false; }; }, deps);
  return state;
}
...
export default function Prices(){
  ...
  const { chart, stats } = useMemo(()=>{
    ...
    avg5.forEach(d => {
      ...
      row.avg5 = d.price; map.set(d.week, row);
    });
    const rows = Array.from(map.values()).sort((a,b)=>a.week-b.week);

    const prices = current.map(d=>d.price).filter(v=>Number.isFinite(v));
  ...
  function exportCSV(){
    const rows = (chart ?? []).map(r => ({
      Year: year, Commodity: commodity, Market: market,
      Week: r.week, Price: r.current ?? "", Avg5: r.avg5 ?? ""
    }));
    ...
    <div className="card">
      ...
      <div className="controls">
        ...
        <div>
          ...
          <input type="number" value={year} onChange={e=>setYear(+e.target.value||thisYear)} />
        </div>
        <label style={{display:"inline-flex",alignItems:"center",gap:8}}>
          <input type="checkbox" checked={showAvg} onChange={e=>setShowAvg(e.target.checked)}/> 5-yr avg
        </label>
        <label style={{display:"inline-flex",alignItems:"center",gap:8}}>
      ...
      {error && <p className="subtle mt-3" style={{color:"#b91c1c"}}>Failed to load data. Showing nothing until it recovers.</p>}
      ...
      {!loading && (
        <>
          <div className="mt-4" style={{height:380}}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chart}>
                ...
                <Tooltip />
                <Legend />
                <ReferenceLine x={13} stroke="#999" strokeDasharray="3 3" label="Mid-season" />
                <Line type="monotone" dataKey="current" name={String(year)} strokeWidth={2} dot={false}/>
                {showAvg && <Line type="monotone" dataKey="avg5" name="5-yr avg" strokeWidth={2} strokeDasharray="6 6" dot={false}/>}
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="mt-3" style={{display:"flex",gap:12,flexWrap:"wrap"}}>
            ...
            <span className="badge">YTD: {stats.change!=null ? stats.change.toFixed(1)+"%" : ""}</span>
            {data?.source === "synthetic" && <span className="badge" title="Backend not reachable">offline model</span>}
          </div>
        </>
      )}
    </div>
