...
function genDemoSeries(name){
  const out=[]; const base=Math.max(5, 25-(name.length%7));
  for(let i=0;i<104;i++){
...
export default function CommodityExplorer(){
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      ...
      try{
        if(mode==="demo"){ if(alive) setSeries(genDemoSeries(commodity)); }
        else {
  ...
  const stats = useMemo(()=>{
    ...
    return { last:+last.toFixed(2), avg:+avg.toFixed(2), max:+max.toFixed(2), min:+min.toFixed(2) };
  },[series]);
    ...
    <div>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-cyan-600/20 to-blue-700/10 p-6 md:p-8">
        <div className="relative z-10">
          <div className="text-cyan-300 text-sm font-semibold tracking-wide">Market Intelligence</div>
          <h1 className="mt-1 text-2xl md:text-3xl font-extrabold">USDA Produce Pricing</h1>
          <p className="mt-2 text-slate-300 max-w-2xl">
            Weekly USDA series with 5-year averages. Toggle Demo/Live in the top-right to connect to your API.
          </p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="card-soft p-3">
          ...
          <select className="select mt-1 w-full" value={commodity} onChange={e=>setCommodity(e.target.value)}>
            {COMMODITIES.map(x=><option key={x} value={x}>{x}</option>)}
          </select>
        </div>
        <div className="card-soft p-3">
        ...
        <div className="card-soft p-3">
          ...
          <div className="mt-1 text-sm">{loading? "Loading" : (err? <span className="text-red-400">{err}</span> : "OK")}</div>
        </div>
      </div>
      ...
      <div className="mt-4 text-xs text-slate-400">
        Live mode expects <code>/ag/commodity?name=&amp;range=</code> to return an array of <code>{{`{ weekLabel, price, avg5y }`}}</code>.
      </div>
    </div>
  );
}