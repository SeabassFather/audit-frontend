...
export default function Mortgage() {
  ...
  async function load(id) {
    ...
    try {
      const j =
        id === "MORTGAGE30US"
          ? await getMortgage30Y()
          : await getFredSeries(id);
      const list = (j?.observations || [])
        .filter((o) => o.value !== "." && o.value != null)
        .slice(-120)
        .map((o, i) => ({ x: o.date, y: +o.value }));
      setObs(list);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="page">
      ...
      <div className="searchBar glass">
        <div className="searchRow">
          <input
            ...
            placeholder="FRED series (e.g., MORTGAGE30US, DGS10, CPIAUCSL)"
          />
          <div className="searchActions">
            <button onClick={() => load(seriesId)} disabled={loading}>
      ...
      <Line points={obs} />
      <p className="mut">
        Pulled via <code>/api/fred/series/{seriesId}</code>. Put your{" "}
        <code>FRED_KEY</code> in <code>.env</code> for full access.
      </p>
    </div>
  );
}
