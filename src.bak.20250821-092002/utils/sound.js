...
export function playClick() {
  ...
  const e = envGain(0.1, 0.002, 0.05, 0.05);
  o.type = "square";
...
export function playSuccess() {
  ...
  const e1 = envGain(0.25, 0.004, 0.12, 0.12);
  const o2 = audio.createOscillator();
  const e2 = envGain(0.25, 0.004, 0.12, 0.12);
  o1.type = "sine";
  o2.type = "sine";
  const t0 = audio.currentTime;
  ...
  o1.connect(e1.g);
  e1.g.connect(audio.destination);
  o2.connect(e2.g);
  e2.g.connect(audio.destination);
  o1.start();
  o2.start();
  o1.stop(e1.tEnd);
  o2.stop(e2.tEnd);
