...
function Login({ onOK }) {
  ...
  const go = async () => {
    setErr("");
    try {
      const r = await apiAuthLogin({ email, password });
      localStorage.setItem("token", r.token || "ok");
      onOK();
    } catch (e) {
      setErr(String(e));
    }
  ...
  return (
    <div className="page-card" style={{ maxWidth: 440 }}>
      ...
      />
      <button className="btn btn-accent" style={{ marginTop: 8 }} onClick={go}>
        Login
      </button>
...
function MarketShell() {
  ...
  const search = async () => {
    setErr("");
    try {
      const r = await apiPost("/api/market/search", q);
      setRows(r.items || []);
    } catch (e) {
      setErr(String(e));
    }
  ...
  return (
    <div className="grid" style={{ gap: 12 }}>
        ...
        <div className="row">
          ...
          <label>
            ...
            <input
              className="input"
              value={q.minLoad}
              onChange={(e) => upd("minLoad", e.target.value)}
            />
          </label>
          <label>
            Price Max (USD)
      ...
      <div className="page-card">
        {rows.length === 0 ? (
          <small className="muted">No results yet.</small>
        ) : (
