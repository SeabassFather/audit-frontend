...
$patterns = @('type=module','/src/main[dot]jsx','<script type=module')
$foundAny = $false
...
if ($Fix) {
  ...
  $new = ($orig -split "`n") | Where-Object { $_ -notmatch 'type=module.*?/src/main[dot]jsx' }
...
Action: removed `<script type=module src="/src/main[dot]jsx"></script>` lines.
"@
